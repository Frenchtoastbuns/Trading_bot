...
def get_stock_data(symbol: str, period: str = None, start: str = None, end: str = None):
    try:
        t = yf.Ticker(symbol)
...
def prepare_full_features(df: pd.DataFrame):
    feature_columns = [
...
def select_best_features(X_train, y_train, original_cols):
...
def train_model(model, train_loader, val_loader, epochs, patience, pos_weight):
...
def load_ensemble_models(num_models: int):
    ensemble = []
...
def predict_with_ensemble(ensemble_models, scaler, row, feature_cols):
    all_probs = []
...
def evaluate_ensemble(ensemble_models, X_test_s, y_test):
    all_probs = []
...
def run_training_and_save():
    ...
    best_features = select_best_features(X.iloc[:tr_end], y.iloc[:tr_end], feature_columns)
    X = X[best_features]
    ...
    pos_weight = torch.tensor([num_neg / num_pos if num_pos > 0 else 1.0], dtype=torch.float32).to(device)

    for i in range(NUM_MODELS):
        model = EnhancedStockNN(input_size=len(best_features))
        model.to(device)
        model = train_model(model, train_loader, val_loader, EPOCHS, PATIENCE, pos_weight)
        torch.save({'model_state_dict': model.state_dict(), 'feature_columns': best_features}, f'model_{i}.pth')
...
def backtest_strategy(start_date: str, end_date: str):
    if not os.path.exists('feature_scaler.joblib') or not os.path.exists('model_0.pth'):
        print("Model/scaler not found. Please train first.")
        return
    ...
    print("Final Portfolio Value:", portfolio_history.iloc[-1])
    print("Total Return:", f"{total_return:.2%}")
    print("Annualized Return:", f"{annualized_return:.2%}")
