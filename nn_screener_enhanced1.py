...
# --- Trading Config ---
MAX_POSITION_SIZE = 0.05      # Base size for a trade at minimum confidence
MIN_CONFIDENCE = 0.6          # Minimum probability to consider a trade
## NEW: DYNAMIC SIZING PARAMETER
MAX_CONFIDENCE_MULTIPLIER = 2.0 # Allow a high-confidence trade to be up to 2x the base size
    ...
    for date, daily_data in backtest_data.groupby(level=0):
        ...
        for _, row in daily_data.iterrows():
            ...
            if confidence > MIN_CONFIDENCE:
                ...
                base_position_value = portfolio_history[date] * MAX_POSITION_SIZE
                
                # Scale size based on how much confidence exceeds the minimum
                # The scale ranges from 1.0 (at MIN_CONFIDENCE) up to MAX_CONFIDENCE_MULTIPLIER (at 1.0 confidence)
                confidence_range = 1.0 - MIN_CONFIDENCE
                if confidence_range > 0:
                    confidence_scale = 1 + ((confidence - MIN_CONFIDENCE) / confidence_range) * (MAX_CONFIDENCE_MULTIPLIER - 1)
                else:
                    confidence_scale = 1.0
                
                target_position_value = base_position_value * confidence_scale
                
                shares_to_buy = int(target_position_value / row['close'])
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        ...
        backtest_strategy(start_date="2023-01-01", end_date="2024-01-01")
